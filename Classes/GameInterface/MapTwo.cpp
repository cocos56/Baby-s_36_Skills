...
bool MapTwo::init()
{
	...
	MenuItemImage *back=MenuItemImage::create("CloseSelected.png","CloseSelected.png",CC_CALLBACK_0(MapTwo::backTwo,this));
	...
	int y3=map3.at("y").asInt();
	 herohrea=Sprite::create("walk_1.png");
...
void MapTwo::update(float f)
{
	...
	switch (rocker2->rocketDirection)
		{
			case 1:
				animation::SetAnimation("walk.plist", "walk.png", "walk_",5, rocker2->rocketRun,herohrea,1);
				 p2= movePlayer(Vec2( herohrea->getPosition().x +2, herohrea->getPosition().y),herohrea->getPosition());
		        herohrea->setPosition(p2);
				break;
			case  2:
				animation::SetAnimation("walk.plist", "walk.png", "walk_", 5, rocker2->rocketRun,herohrea, 2);
				 p2= movePlayer(Vec2(herohrea->getPosition().x,herohrea->getPosition().y +2),herohrea->getPosition());
		        herohrea->setPosition(p2);
				break;
			case 3:
				animation::SetAnimation("walk.plist", "walk.png", "walk_", 5, rocker2->rocketRun,herohrea,3);
	            p2= movePlayer(Vec2( herohrea->getPosition().x -2,  herohrea->getPosition().y),herohrea->getPosition());
		        herohrea->setPosition(p2);
				break;
			case 4:
				animation::SetAnimation("walk.plist", "walk.png", "walk_", 5, rocker2->rocketRun,herohrea,4);
				 p2= movePlayer(Vec2( herohrea->getPosition().x,  herohrea->getPosition().y -2),herohrea->getPosition());
		        herohrea->setPosition(p2);
				break;
			default:
...
void MapTwo::heroRound(float d)
{
    ...
    //auto func=CallFuncN::create(CC_CALLBACK_1(MapTwo::heroRound, this));
    auto ani= animation::getAnimation("walk.plist", "walk.png", "walk_", 5,heroBad,3);
	auto action = Animate::create(ani);
	heroBad->runAction(Sequence::create(action,move, NULL));
	if(kj==0)
