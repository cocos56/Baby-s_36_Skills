...
void MapFive::update(float f)
{
	...
	if (rectA.intersectsRect(rectC)&&chu->isVisible()) { backFive(); }
...
void MapFive::heroRound(float d)
{
	...
	auto action = Animate::create(ani);
	heroBad5->runAction(Sequence::create(action,move, NULL));
	if(kj==0){ schedule(schedule_selector(MapFive::heroRound),0.3); }
	else{ unschedule(schedule_selector(MapFive::heroRound)); }
}
